...
class FlashGenerator():
    ...
    def __parse_content(self, template_path, check_overflow: bool=True):
        ...
        with open(template_path, "rb") as template:
            content_dict = json.load(template, object_pairs_hook=OrderedDict)
        ...
        if content_dict.get('sections') is not None:
            flash_offset = 0
            section_start_align = self.target.get_target_property('section_start_align')
            section_size_align = self.target.get_target_property('section_size_align')
