...
import gapylib.chips.gap.gap9_v2.rtl_runner
...
class Target(gapylib.chips.gap.target.Target):
    ...
    runner
        If given, overwrites the default runner
    """

    def __init__(self, parser: argparse.ArgumentParser, options: list, runner=None):
    ...
    def append_args(self, parser: argparse.ArgumentParser):
        ...
        if self.runner is None:
            if args.platform == 'rtl':
                self.runner = gapylib.chips.gap.gap9_v2.rtl_runner.Runner(self)

            elif args.platform == 'board':
