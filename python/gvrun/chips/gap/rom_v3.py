"""Provides section template for gap rom v3 (starting from gap10)"""
...
class RomHeader(CStruct):
    ...
    def __init__(self, name, parent: CStructParent):
        ...
        # Offset in the flash to the first binary
        self.add_field('offset_0', 'I')
        # Offset in the flash to the second binary
        self.add_field('offset_1', 'I')
        # Padding to align next field on 16 bytes
        self.add_field_array('padding_0', 1*4)

        # Index of the binary to be loaded
        self.add_field('binary_index', 'I')
        self.add_field('padding_1', 'I')
        self.add_field('padding_2', 'I')
        self.add_field('padding_3', 'I')
...
@dataclasses.dataclass
class RomBinaryHeader(CStruct):
    """
    Class for generating rom sub-section containing a binary header.
...
class Xip():
    ...
    def __init__(self, flash: Flash, page_size_cmd, flash_base, nb_pages, vaddr):
        ...
        self.xip_l2_addr = 0x1c250000 - self.xip_l2_nb_pages*self.xip_page_size
...
class RomFlashSection(FlashSection):
    ...
    def __init__(self, parent: Flash, name: str, section_id: int):
        ...
        self.declare_property(name='firmware_min_size', value=0x0,
            description="Minimum size to reserve for the firmware."
        )
    ...
    def set_content(self, offset: int, content_dict: dict):
        ...
        if self.binary is not None:
            ...
            # We need to compute the size taken for the binary in case there is a required
            # minimum size
            firmware_size = 0

            binary = RomBinaryHeader('Binary header', parent=self.top_struct)
            firmware_size += binary.get_size()
            ...
            for __ in binary_segments:
                ...
                # Since the segments headers are stored along with the segments, we need to account
                # them in the firmware size
                firmware_size += segment.get_size()
            ...
            for binary_segment in binary_segments:
                ...
                if xip.is_xip_segment(binary_segment.base):
                    ...
                    firmware_size += len(binary_segment.data)

            # Check if firmware must have a minimum size, and if so, maybe add padding
            firmware_min_size = int(self.get_property('firmware_min_size'))
            if firmware_min_size != 0:

                # We add padding for binary 0 only if it is smaller than required size
                diff = firmware_min_size - firmware_size
                if diff > 0:
                    padding = CStruct('binary_0_padding', parent=self.top_struct)
                    padding.add_field_array('binary_0_padding', diff)

                # And we always add padding for binary 1 so that room is reserved and we can
                # properly set the offset to it in the header
                padding_binary_1 = CStruct('binary_1_padding', parent=self.top_struct)
                padding_binary_1.add_field_array('binary_1_padding', firmware_min_size)
            ...
            # known yet
            self.header.set_field('offset_0', binary.get_offset())
            self.header.set_field('binary_index', 0)

            binary.set_field('nb_segments', len(self.binary.segments))
            binary.set_field('entry', self.binary.entry)
            xip.fill_header(binary)
            ...
            for i, binary_segment in enumerate(binary_segments):
                ...
                # addresses are relative to the start of the binary
                # This allows romv2 sections to be move anywhere in memory
                addr_offset = segment.get_field('data').get_offset() - binary.get_offset()
            ...
            # If firmware minimum size is specified, room has been reserved for second binary
            # and we need to make the second offset points to it
            if firmware_min_size != 0:
                self.header.set_field('offset_1', padding_binary_1.get_offset())
    ...
    def get_image(self) -> bytes:
        ...
        key_string = self.get_flash().get_target().get_target_property('rom.security.aes_key')
        iv_string = self.get_flash().get_target().get_target_property('rom.security.aes_iv')
        if key_string is not None:
            #key_string = key_string.encode('utf-8')
            #iv_string = iv_string.encode('utf-8')
            key = bytearray()
            for byte in key_string:
                hex_byte = int(byte, 16)
                key.extend(hex_byte.to_bytes(length=1,byteorder='big'))
            aes_iv = bytearray()
            for byte in iv_string:
                hex_byte = int(byte, 16)
                aes_iv.extend(hex_byte.to_bytes(length=1,byteorder='big'))
            ctr = Counter.new(64, prefix=aes_iv, little_endian=False, initial_value=0)
            cipher = AES.new(key, AES.MODE_CTR, counter=ctr)
            result_cypher = bytearray()
            result_cypher += cipher.encrypt(result)
            return result_cypher
        return result
